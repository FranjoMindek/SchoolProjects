...
Animal* createDogs(const char **names, int n);
//funkcije glasanja
...
int main(){
    ...
    Animal* Dogs = createDogs(names, 4);
    for(int i = 0; i<4; i++){
...
Animal* createDogs(const char **name, int n){
    Animal *memoryForAll = (Animal*) malloc(n*sizeof(Animal));
    for(int i = 0; i<n; i++){
        constructDog(memoryForAll + i, name[i]);
    }
    return memoryForAll;
}
