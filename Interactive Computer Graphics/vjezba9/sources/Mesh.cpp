...
//        std::cout << mMinMax.x[0] << " " << mMinMax.x[1] << std::endl;
    ...
    if (scene->HasMaterials()) {
        for (int i = 0; i < scene->mNumMaterials; i++) {
            ...
            std::cout <<  "[" << i << "]" << naziv.C_Str() << "  " << scene->mMaterials[i]->mProperties << std::endl;
            ...
            if (AI_SUCCESS == scene->mMaterials[i]->Get(AI_MATKEY_TEXTURE(aiTextureType_DIFFUSE, 0), texturePosition)) {
                ...
                std::cout << "Toliko:" << texturePosition.C_Str() << std::endl;
                std::string tex_path(dir);
                std::string pos = texturePosition.C_Str();
                pos.replace(8,2,"/");
                tex_path.append(pos);

                std::cout << "Radim nesto " << tex_path << std::endl;
