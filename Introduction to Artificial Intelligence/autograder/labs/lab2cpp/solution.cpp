...
void AutomatedReasoning::findContradiction(){
    ...
    while(more){
        ...
        for(auto first = support_clauses.rbegin(); first!=support_clauses.rend(); first++){ //zasada izostaje base/base dio -> nije potreban jer ako je potrebno ne valjda ionako ulaz
