...
def right_rotation(node: Node, root: Node) -> Node:
    ...
    """
    rotator = node.left # TODO: The child
    new_root = root # TODO: The resulting root (initially set to...)
    if rotator is None:
        return new_root
    parent = node.parent
    if parent is None:
    ...
    # TODO: We have a new root!
        ...
        if parent.left == node: parent.left = rotator
        if parent.right == node: parent.right = rotator
