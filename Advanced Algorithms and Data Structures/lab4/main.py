...
def is_in_window(point, qx1: float, qx2: float, qy1: float, qy2: float) -> bool:
    """
...
class IntervalTree:
    ...
    def _query(self, node: IntervalNode, qx1: float, qx2: float, qy1: float, qy2: float) -> list:
        ...
        if is_in_range(qx1, qx2, node.x_median):
            for dot in node.tree_left:
                if qy1 < dot['point'][1] < qy2 and dot['point'][0] < qx2:
                    iv.add(dot['interval'])
            for dot in node.tree_right:
                if qy1 < dot['point'][1] < qy2 and dot['point'][0] > qx1:
                    iv.add(dot['interval'])
            move = 'both'
        elif node.x_median < qx1:
            for dot in node.tree_right:
                if qy1 < dot['point'][1] < qy2 and dot['point'][0] > qx1:
                    iv.add(dot['interval'])
            move = 'right'
        elif node.x_median > qx2:
            for dot in node.tree_left:
                if qy1 < dot['point'][1] < qy2 and dot['point'][0] < qx2:
                    iv.add(dot['interval'])
            move = 'left'
